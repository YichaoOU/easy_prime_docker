...
def get_current_pegRNA_table_title_and_download_links(df,jid):
	...
	header = dbc.FormGroup(
		[
			...
			html.A(html.Button('Download Top5 predictions',className="btn btn-dark"),href="results/{}".format(urlquote("%s_Top5_rawX_pegRNAs.csv.gz"%(jid))),target="_blank",style={"margin-right":10}),
			html.A(html.Button('Download all',className="btn btn-dark"),href="results/{}".format(urlquote("%s_rawX_pegRNAs.csv.gz"%(jid))),target="_blank",style={"margin-right":1}),
...
def run_easy_prime_backend(vcf,jid,parameters):
	...
	# add top 5, 7-26-2021
	df2 = df_all.drop_duplicates("sample_ID")
	df2 = df2.sort_values("predicted_efficiency",ascending=False)
	top5_index = df2.head(n=5).sample_ID.tolist()
	df3 = df_all[df_all.sample_ID.isin(top5_index)]
	df3.to_csv("results/%s_Top5_rawX_pegRNAs.csv.gz"%(jid),index=False,compression="gzip")
...
def df2bedjs(df,output):
	...
	return '''{"type":"bedj","url":"http://easy-prime.cc/results/%s.bed.gz","stackheight":20,"stackspace":1,"name":"%s"},'''%(output,track_name)
